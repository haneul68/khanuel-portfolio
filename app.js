...
function gnBancInterviewProjectTemplate() {
  return {
    ...
    portfolioTemplateVersion: 7,
    sections: [
      { type: "overview", title: "프로젝트 개요", text: "GN Banc는 캐릭터를 배치하면 자동 전투가 진행되고, 스테이지 클리어와 던전 보상을 통해 캐릭터와 유물을 성장시키는 모바일 방치형 수집 RPG입니다. 자동 전투 자체보다 플레이어가 다시 접속하고 성장 결과를 확인하고 싶어지는 반복 구조를 직접 설계하는 데 목적을 두었습니다." },
      { type: "features", title: "한눈에 보기", features: ["자동 전투 루프", "스테이지 진행", "성장/보상 구조", "Firebase 저장", "오프라인 보상", "UI 피드백"] },
      { type: "gallery", title: "플레이 화면", images: ["assets/project-gn-banc-title.png"] },
      { type: "architecture", title: "전체 시스템 구조", note: "구조도는 플레이 흐름과 시스템 책임을 한눈에 이해할 수 있도록 정리한 지도입니다. 전투 흐름은 StageManager가 통제하고, 실제 기능은 각 Manager와 Character 계열 클래스가 나눠 담당합니다.", flowTitle: "Play Loop", systemTitle: "Runtime Modules", flow: ["Start", "Prepare", "Battle", "Reward", "Growth"], modules: ["BaseManager", "StageManager", "Spawner", "Player / Monster", "DataManager", "Firebase_Manager"], domains: ["Combat", "Growth", "Reward", "UI", "Save"] },
      { type: "features", title: "담당 구현", features: ["StageManager 기반 전투 상태 전환", "플레이어와 몬스터 전투 처리", "보상 지급과 성장 데이터 관리", "Firebase 저장/불러오기", "던전과 오프라인 보상 UI"] },
      { type: "text", title: "기술 포인트", text: "반복 생성되는 몬스터와 보상 흐름은 관리 지점이 흩어지면 유지보수가 어려워지기 때문에 Manager 계열로 책임을 나누고, 전투 상태와 데이터 저장 흐름을 분리했습니다. 또한 방치형 게임 특성상 보상이 즉시 읽혀야 하므로 결과 UI와 성장 피드백을 명확하게 보여주는 데 집중했습니다." },
      { type: "features", title: "문제 해결 경험", features: ["전투 상태가 UI와 데이터 처리에 섞이지 않도록 StageManager 중심으로 정리", "반복 생성 객체는 풀링 구조로 관리", "캐릭터/아이템/진행 데이터는 DataManager 중심으로 분리", "접속 종료 후 보상 흐름은 Firebase 저장 구조와 연결"] },
      { type: "text", title: "배운 점", text: "기능을 각각 구현하는 것보다, 전투 준비부터 보상과 성장까지 어떤 순서로 이어져 플레이어 경험을 만드는지 설계하는 것이 중요하다는 점을 배웠습니다. 또한 방치형 게임에서는 자동 전투가 끝난 뒤에도 성장 결과가 명확히 남고, 다음 목표가 자연스럽게 보일 때 플레이를 계속 이어가고 싶어진다는 점을 확인했습니다." },
      { type: "text", title: "보완할 부분", text: "향후에는 실제 플레이 데이터를 기준으로 성장 속도와 보상 밸런스를 조정하고, 전투 연출과 캐릭터 수집 동기를 더 강화할 계획입니다." }
    ]
  };
}
...
function upgradeGnBancProject(data) {
  ...
  if ((current.portfolioTemplateVersion || 0) >= 7 && current.thumb && current.heroImage) return false;
  data.projects[index] = mergeProjectDefaults(current, gnBancInterviewProjectTemplate());
  data.projects[index].portfolioTemplateVersion = 7;
  return true;
}
...
function renderArchitectureSection(section) {
  ...
  const systems = Array.isArray(section.systems) ? section.systems : [];
